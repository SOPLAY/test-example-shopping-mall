...
describe('debounce 단위 테스트', () => {
  // 테스트 코드는 비동기 타이머와 무관하게 동기적으로 실행
  // -> 비동기 함수가 실행되기 전에 단언이 실행됨
  // 타이머 모킹!
  beforeEach(() => {
    // 타이머 모킹 -> 0.3 초 흐른것으로 타이머 조작 -> spy 함수 호출 확인
    // teardown 에서 모킹 초기화 -> 다른 테스트에 영향이 없어야 함

    // 타이머 모킹도 초기화 필수!
    // 3rd 파티 라이브러리, 전역의 teardown에서 타이머에 의존하는 로직 -> fakeTimer 로 인해 제대로 동작하지 않음
    vi.useFakeTimers();
  ...
  it('연이어 호출해도 마지막 호출 기준으로 지정된 타이머 시간이 지난 경우에만 함수가 호출된다.', () => {
    ...
    // 다섯번을 호출했지만 실제 spy 함수는 마지막 호출에서만 호출되어야 한다.
